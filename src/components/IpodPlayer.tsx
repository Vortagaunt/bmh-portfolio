...
const COVERS = Array.from({ length: 13 }, (_, i) => ({
  ...
  cover: `/images/cover-flow/album${i + 1}.webp`,
}));
const COVER_TRACKS: Track[] = COVERS.map((c) => ({
  title: c.title, artist: c.artist, art: c.cover, src: BG,
}));

const MENU = ["Cover Flow", "Cascade Creations", "Now Playing"];
...
type View = "menu" | "cascade" | "coverflow" | "now";
...
export function IpodPlayer() {
  ...
  useEffect(() => {
    ...
    const step = (dir: 1 | -1) => {
      ...
      else if (c.top === "coverflow") setCf((p) => Math.min(COVERS.length - 1, Math.max(0, p + dir)));
      else {
        const a = audioRef.current;
        if (a && a.duration) {
    ...
    const onUp = (e: PointerEvent) => {
      ...
      if (dist < centerR) {
        ...
        else if (cur.top === "coverflow") playAt(COVER_TRACKS, cur.cf);
        else togglePlay();
        return;
      }
      const ang = (Math.atan2(dy, dx) * 180) / Math.PI;
      if (ang >= -135 && ang < -45) pop();                 // top — MENU
      else if (ang >= -45 && ang < 45) {                   // right — next
        if (cur.top === "now") skip(1); else step(1);
      } else if (ang >= 45 && ang < 135) togglePlay();     // bottom — play/pause
      else {                                               // left — prev
        if (cur.top === "now") skip(-1); else step(-1);
      }
    };
  ...
  const titleBar = top === "menu" ? "iPod" : top === "cascade" ? "Cascade Creations" : top === "coverflow" ? "Cover Flow" : "Now Playing";
                    ...
                    return (
                      <div key={c.cover} className="absolute"
                        style={{
                          ...
                          transformStyle: "preserve-3d", zIndex: 100 - abs, opacity: abs > 4 ? 0 : 1,
                          transition: "transform .42s cubic-bezier(.2,.7,.1,1), opacity .42s ease",
                        }}>
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img src={c.cover} alt="" className="block h-full w-full" style={{ objectFit: "cover", borderRadius: "1cqw", boxShadow: "0 2cqw 5cqw rgba(0,0,0,.6)" }} draggable={false} />
                        {/* reflection */}
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img src={c.cover} alt="" aria-hidden className="absolute left-0 block h-full w-full"
                          style={{ top: "100%", objectFit: "cover", transform: "scaleY(-1)", opacity: 0.28,
