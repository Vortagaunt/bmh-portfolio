...
const W = 460;
const H = Math.round(W * (420 / 540)); // ~358px — maintain mac aspect ratio

export function IntroOverlay() {
  const [phase, setPhase] = useState(0);
  const pathRef = useRef<SVGPathElement>(null);
  const [macPos, setMacPos] = useState({ top: 0, left: 0, width: W, height: H });
  const [moving, setMoving] = useState(false);

  useEffect(() => {
    // Center the mac on screen
    setMacPos({
    ...
    const timers = [
      ...
      // Hold — hello fully drawn
      setTimeout(() => setPhase(3), 2800),
      // Mac flies to its hero position
      setTimeout(() => {
        setPhase(4);
      ...
      // Overlay fades out, hero mac revealed
      setTimeout(() => {
        setPhase(5);
        const target = document.getElementById("hero-mac-target");
        if (target) {
  ...
  const macOpacity = phase >= 5 ? 0 : phase >= 1 ? 1 : 0;
  ...
  return (
    <>
      ...
      {/* Mac — moves from center to hero position */}
      <div
        ...
        style={{
          ...
          transform: !moving && phase >= 1 ? "translateY(0px)" : !moving ? "translateY(28px)" : undefined,
        }}
      >
        <div className="relative w-full h-full">
          ...
          {/* Hello drawn onto the mac screen */}
          <div
            className="absolute"
            style={{
              left: "20%",
